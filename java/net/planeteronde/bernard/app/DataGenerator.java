package net.planeteronde.bernard.app;
...
import net.planeteronde.bernard.backend.OrderRepository;
import net.planeteronde.bernard.backend.PickupLocationRepository;
import net.planeteronde.bernard.backend.ProductRepository;
import net.planeteronde.bernard.backend.ProjetRepository;
import net.planeteronde.bernard.backend.UserRepository;
...
import net.planeteronde.bernard.backend.data.entity.Order;
import net.planeteronde.bernard.backend.data.entity.OrderItem;
import net.planeteronde.bernard.backend.data.entity.PickupLocation;
import net.planeteronde.bernard.backend.data.entity.Product;
...
public class DataGenerator implements HasLogger {
	...
	private static final String[] NOM_PROJETS = new String[] { "Riviera", "Paimpol", "Mimosa" };
	private static final String[] FILIALE_PROJETS = new String[] { "Sud-Est", "Grand Ouest", "Sud-Est" };
	...
	private User respPortfolio;
	private User respProjet;

	@Bean
	public CommandLineRunner loadData(OrderRepository orderRepository, UserRepository userRepository,
			ProductRepository productRepository, ProjetRepository projetRepository, PickupLocationRepository pickupLocationRepository,
		...
		return args -> {
			...
			getLogger().info("... generating projets");
			createProjets(projetRepository);
	...
	private List<HistoryItem> createOrderHistory(Order order) {
		...
		HistoryItem item = new HistoryItem(getRespProjet(), "Order placed");
		...
		if (order.getState() == OrderState.CANCELLED) {
			item = new HistoryItem(getRespProjet(), "Order cancelled");
			...
			item = new HistoryItem(getRespPortFolio(), "Order confirmed");
			...
			if (order.getState() == OrderState.PROBLEM) {
				item = new HistoryItem(getRespPortFolio(), "Can't make it. Did not get any ingredients this morning");
				...
				item = new HistoryItem(getRespPortFolio(), "Order ready for pickup");
				...
				if (order.getState() == OrderState.DELIVERED) {
					item = new HistoryItem(getRespPortFolio(), "Order delivered");
	...
	private User getRespPortFolio() {
		return respPortfolio;
	}

	private User getRespProjet() {
		return respProjet;
	}
	...
	private void createProjets(ProjetRepository projetRepository) {
		for (int i = 0; i < NOM_PROJETS.length; i++) {
			Projet projet = new Projet();
			projet.setNom(NOM_PROJETS[i]);
			double doubleMontant = 2.0 + random.nextDouble() * 100.0;
			projet.setMontant((int) (doubleMontant * 100.0));
		//	projet.setFiliale(FILIALE_PROJETS[i]);
	...
	private void createUsers(UserRepository userRepository, PasswordEncoder passwordEncoder) {
		respPortfolio = userRepository.save(new User("dupont@estp.com", "Jean Dupont", passwordEncoder.encode("tintin"), Role.RESP_PORTFOLIO));
		User user = new User("dupond@estp.com", "Jacques Dupond", passwordEncoder.encode("milou"), Role.RESP_PROJET);
//		user.setLocked(true);
		respProjet = userRepository.save(user);
		user = new User("aaa", "Francis Claude", passwordEncoder.encode("aaa"), Role.RESP_SYSTEME);
		user.setLocked(true);
